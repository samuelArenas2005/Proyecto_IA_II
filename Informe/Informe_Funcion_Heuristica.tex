...
Knight Energy es un juego adversarial entre dos caballos sobre un tablero de ajedrez. La máquina juega con el caballo blanco (\textit{white}) y el jugador humano con el caballo negro (\textit{black}). En la interfaz del juego, esta representación lógica se acompaña con una identidad visual: el caballo del jugador se muestra dorado y el caballo controlado por la inteligencia artificial se muestra con apariencia plateada, manteniendo internamente la distinción blanco/negro para el cálculo de turnos, puntos y movimientos.
...
\begin{center}
\href{https://github.com/samuelArenas2005/Proyecto_IA_II}{samuelArenas2005/Proyecto\_IA\_II}
\end{center}
...
El siguiente fragmento resume la parte final de la función heurística y muestra cómo se combinan los componentes ponderados. No corresponde a la función completa, ya que en la implementación real también se calculan previamente valores auxiliares como \texttt{star\_options\_diff}, \texttt{energy\_options\_diff} y \texttt{blocked\_score}. En particular, el código completo incluye el cálculo de las estrellas alcanzables por cada jugador, las casillas de energía alcanzables y la penalización o recompensa asociada a que un jugador quede bloqueado sin movimientos efectivos.
...
Además, la separación entre la representación visual y la representación lógica del juego facilita la implementación del algoritmo. Aunque en pantalla el jugador se identifica con el caballo dorado y la IA con el caballo plateado, internamente se conservan los roles \textit{black} y \textit{white}. Esta decisión permite que las funciones de utilidad, heurística y generación de movimientos trabajen con una estructura clara y consistente.
...
\section{Añadido: heurísticas configurables y torneo}
...
También se incorporó un nuevo parámetro llamado control central. Este valor mide qué tan cerca se encuentra cada caballo del centro del tablero. La idea es que un caballo ubicado en posiciones centrales suele tener más posibilidades de movimiento que uno ubicado en bordes o esquinas. Por eso, el control central se calcula como una diferencia entre la centralidad del caballo \textit{white} y la centralidad del caballo \textit{black}. Si la IA está mejor posicionada en el centro, este componente aumenta la evaluación; si el rival tiene mejor control central, la disminuye.
